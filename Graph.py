...
def resizing(roi):  # return roi resized to (350, 600)
    ...
    if (roi.shape[0] / roi.shape[1]) < (7/12):
        dims = (300, (roi.shape[0] * 300) // roi.shape[1])
    else:
        dims = ((175 * roi.shape[1]) // roi.shape[0], 175)
...
for i in range(472, len(a)):
    ...
    save('D:/Pre-processing/data_roi_medulla_175x300_wo_exp_std/roi_%s' % a[i][16:24] + '_' + '%s.npy' % i, ROI)
    save('D:/Pre-processing/data_roi_medulla_175x300_wo_exp_std/medulla_%s' % a[i][16:24] + '_' + '%s.npy' % i, medulla)
