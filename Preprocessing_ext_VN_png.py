######## EXTERNAL DATA FROM VIETNAM JPG ###########



import nrrd, os, pydicom, cv2, copy, timeit
import numpy as np
...
a = []
b = []
a_l = os.listdir('D:/external/')
b_l = os.listdir('D:/label/')
for i in a_l:
    p = 'D:/external/' + i
    a.append(p)
for i in b_l:
...
for i in range (249, len(a)): 
        ...
        if num > 0:
            ...
            if num_pixel == 0:
                ...
                plt.axis('off')
                plt.savefig('D:/data/ext_VN/%s' %a[i][12:] + '_%s.png' %i, bbox_inches='tight',pad_inches = 0)
    
                stop = timeit.default_timer()
                print ('FINAL STEPPPPP of %s' %i, 'with time:', stop-start)
        else:
            small.append ('mask = 0:' + str(i))
print (small)
